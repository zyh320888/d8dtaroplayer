...
import { BrowserRouter, Routes, Route, useNavigate, HashRouter, MemoryRouter } from "react-router-dom";
...
import UserStore, { getStore as userGetStore, setStore as userSetStore } from "./../player/user_store";
import { flattenTree, findNodeById, filterObjectByProperty, getFrontCode, hasHttpOrHttps } from './../player/player_utils';
import playerEventEmitter from './../player/player_global_events';
import runJs from "./../player/runJs"
import Api from './../player/player_api';
...
const OSS_HOST = 'https://d8doss.y2o.me';//CONFIG.OSS_HOST;
...
function getEleNode({ _this, $refs, treeId, style, props, child, findChild, output, type }) {
    ...
    else if (type === "textarea") output.push(_this.getACommonNode({ $refs, treeId, style, props, Element: TextArea }));
    // else if (type === "picker") output.push(_this.getACommonNode({ $refs, treeId, style, props, Element: Picker }));
    else if (type === "picker") output.push(NPickerNode({ d8dprops: { _this: _this, $refs: null, treeId, style, props, child } }));
    ...
    else if (type === "cell.group") output.push(_this.getACommonNode({ $refs: null, treeId, style, props, child, Element: Cell.Group }));
    ...
    else if (type === "uploader") output.push(_this.getAUploadNode({ $refs, treeId, style, props, child }));
    ...
    else if (type === "dialog") output.push(<ADialogNode key={treeId} {...{ _this: _this, $refs, treeId, style, props, child }} />);
    else if (type === "popup") output.push(<APopupNode key={treeId} {...{ _this: _this, $refs, treeId, style, props, child }} />);
...
function FrontComponent(superProps) {
    ...
    // console.log('FrontComponent',treeId, eleProps);
    ...
    const onOutputNodes = (forItemNode, index) => {
        ...
        setOutputNode(prevState => {
            if (prevState?.length > 0) {
                ...
                prevState.splice(index, 1, React.cloneElement(forItemNode, {
                    ...forItemNode.props,
    ...
    useEffect(() => {
        ...
        if (Array.isArray(dataSource)
            ...
            dataSource.map((item, index) => {
                ...
                const treeCode = FNodeCode.children.map((item, index) => {
                    item.props = {
                        ...item.props,
                        ...rest,
                        key: item.id + index
...
function ADialogNode({ _this, $refs, treeId, style, props, child }) {
...
function APopupNode({ _this, $refs, treeId, style, props, child }) {
...
function BCommonNode(superProps) {
    ...
    // if (treeId.toLowerCase().indexOf('navbar_') > -1 && (rest.style.marginTop === undefined || rest.style.marginTop === '')){
    //     rest.style.marginTop = Taro.getSystemInfoSync().statusBarHeight;
    // }
                ...
                return {
                    callMethod: (method, param, callback) => {
                        ...
                        if (method == 'getConfig') {
                            ...
                            var val = '';
                            if (param == 'apiHost') {
                                let ENV = getStore('APP_CONFIG').ENV;
                ...
                return {
                    callMethod: (method, param) => {
                        ...
                        if (method == 'navigate') {
                            ...
                            if (param.path === '/') {
                                path = param.path;
                                Taro.navigateTo({ url: `/route/index?${objectToQueryString(query)}` })
                            } else {
                                path = param.path + '/';
                                Taro.navigateTo({ url: `/route/index?_=#${path}${objectToQueryString(query)}` })
                            }
                            ...
                            return getStore('PAGE_URL_PARAMS');
                        }
...
class UICommon extends PlayerUI {
    ...
    propsAttrJsHandler(props) {
        ...
        Object.keys(props).map(name => {
            ...
            else if (name.includes('nativeType') && props.hasOwnProperty(name) && props[name] !== undefined) eleProps['formType'] = props[name];
            else if (name.includes('openType') && props.hasOwnProperty(name) && props[name] !== undefined) eleProps[name] = props[name];
        })
...
class Player extends UICommon {
    ...
    componentDidMount() {
        if (!this.loaded) {
            ...
            if (process.env.TARO_ENV !== 'h5') {
                const appCode = Taro.getStorageSync('treeArr')
    ...
    onLoad(options) {
        ...
        setStore('PAGE_URL_PARAMS', options);

        //   setTimeout(()=>{
        //     playerEventEmitter.emit('ON_NAVIGATE', options.path);
        //   },100)
            ...
            <HashRouter>
                <ConfigProvider
                    ...
                    theme={{
                        ...
                        nutuiNavbarPadding: `${22 + Taro.getSystemInfoSync().statusBarHeight}px 16px 22px 16px`,
                    }}
                >
